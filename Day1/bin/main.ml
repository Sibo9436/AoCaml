let _testf = "input/test.txt"
let _inputf = "input/first.txt"
let words  = [
  "one" , "1";
  "two" , "2";
  "three" , "3";
...
let () = let file  = open_in _inputf
  ...
  in
  let find_pair x  = let just_num =  String.fold_left (fun acc c -> if String.length acc = 0 && is_ascii_number c then  String.make 1 c   else acc ) ""
